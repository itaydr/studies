...
hoare_slots_monitor_t* hoare_slots_monitor_alloc() {
  ...
  if ((monitor->seats_avaliable= hoare_cond_alloc()) == -1){
    goto cleanup;
  }
  
  if ((monitor->all_seats_taken= hoare_cond_alloc()) == -1){
    goto cleanup;
  }
  
  if( (monitor->monitor_mutex = kthread_mutex_alloc()) == -1){
    goto cleanup;
  }
...
cleanup:
  ...
  if (monitor && -1 != monitor->seats_avaliable) {
    hoare_cond_dealloc(monitor->seats_avaliable);
  }
  if (monitor && -1 != monitor->all_seats_taken) {
...
int hoare_slots_monitor_takeslot(hoare_slots_monitor_t* monitor) {
  ...
  if ( 0 == monitor->seats) {
    hoare_cond_wait(monitor->seats_avaliable);
  } 
