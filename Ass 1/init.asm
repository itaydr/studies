...
main(void)
{
  ...
  b5:	75 48                	jne    ff <main+0xff>
      exec("sh", argv);
  b7:	c7 44 24 04 60 0b 00 	movl   $0xb60,0x4(%esp)
  be:	00 
  bf:	c7 04 24 cb 08 00 00 	movl   $0x8cb,(%esp)
  c6:	e8 fd 02 00 00       	call   3c8 <exec>
      printf(1, "init: exec sh failed\n");
  cb:	c7 44 24 04 fc 08 00 	movl   $0x8fc,0x4(%esp)
  d2:	00 
  ...
  e6:	e8 a5 02 00 00       	call   390 <exit>
    ...
    while((wpid=wait(NULL)) >= 0 && wpid != pid)
      printf(1, "zombie!\n");
  eb:	c7 44 24 04 12 09 00 	movl   $0x912,0x4(%esp)
  f2:	00 
  f3:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
  fa:	e8 08 04 00 00       	call   507 <printf>
    if(pid == 0){
      exec("sh", argv);
      printf(1, "init: exec sh failed\n");
      exit(EXIT_STATUS_ERR);
